execute as @n[type=item,distance=..0.5] if data entity @s {Item:{id:"minecraft:glistering_melon_slice"}} run function witchpack:rituals/remove_item

scoreboard players remove @s c_wp_capacity 1

data modify entity @s data.brew append value {id:"minecraft:instant_health",amplifier:0,duration:2}

scoreboard players add @s c_wp_color 16065571
...
tag @s add c_wp_brew_instant_health
