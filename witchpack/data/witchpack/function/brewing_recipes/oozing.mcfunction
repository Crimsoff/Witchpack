execute as @n[type=item,distance=..0.5] if data entity @s {Item:{id:"minecraft:slime_block"}} run function witchpack:rituals/remove_item

scoreboard players remove @s c_wp_capacity 1

data modify entity @s data.brew append value {id:"minecraft:oozing",amplifier:0,duration:4800}

scoreboard players add @s c_wp_color 9960609
