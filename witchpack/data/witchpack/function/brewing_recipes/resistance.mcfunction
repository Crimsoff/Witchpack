execute as @n[type=item,distance=..0.5] if data entity @s {Item:{id:"minecraft:turtle_scute"}} run function witchpack:rituals/remove_item

scoreboard players remove @s c_wp_capacity 1

data modify entity @s data.brew append value {id:"minecraft:resistance",amplifier:0,duration:1200}

scoreboard players add @s c_wp_color 4635977
...
tag @s add c_wp_brew_resistance
