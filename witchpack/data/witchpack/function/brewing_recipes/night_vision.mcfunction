execute as @n[type=item,distance=..0.5] if data entity @s {Item:{id:"minecraft:golden_carrot"}} run function witchpack:rituals/remove_item

scoreboard players remove @s c_wp_capacity 1

data modify entity @s data.brew append value {id:"minecraft:night_vision",amplifier:0,duration:4800}

scoreboard players add @s c_wp_color 12647525
...
tag @s add c_wp_brew_night_vision
