execute as @n[type=item,distance=..0.5] if data entity @s {Item:{id:"minecraft:tropical_fish"}} run function witchpack:rituals/remove_item

scoreboard players remove @s c_wp_capacity 1

data modify entity @s data.brew append value {id:"minecraft:dolphins_grace",amplifier:0,duration:3600}

scoreboard players add @s c_wp_color 12045537
...
tag @s add c_wp_brew_dolphins_grace
