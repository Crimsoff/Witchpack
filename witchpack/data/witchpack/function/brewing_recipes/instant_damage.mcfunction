execute as @n[type=item,distance=..0.5] if data entity @s {Item:{id:"minecraft:pointed_dripstone"}} run function witchpack:rituals/remove_item

scoreboard players remove @s c_wp_capacity 1

data modify entity @s data.brew append value {id:"minecraft:instant_damage",amplifier:0,duration:2}

scoreboard players add @s c_wp_color 10970217
...
tag @s add c_wp_brew_instant_damage
