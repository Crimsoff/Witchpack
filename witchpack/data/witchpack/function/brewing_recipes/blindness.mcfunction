execute as @n[type=item,distance=..0.5] if data entity @s {Item:{id:"minecraft:closed_eyeblossom"}} run function witchpack:rituals/remove_item

scoreboard players remove @s c_wp_capacity 1

data modify entity @s data.brew append value {id:"minecraft:blindness",amplifier:0,duration:1800}

scoreboard players add @s c_wp_color 7431528
